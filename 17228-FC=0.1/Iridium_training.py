...
config = {
    ...
    "preAEV": False,
    "GraphEveryEpochs": 10,
    "OutputFolder": "../../N_vs_error/17228-FC=0.1"
}
...
def validate():
    ...
    model.train(True)
    return hartree2kcalmol(math.sqrt(total_mse / count)), hartree2kcalmol(math.sqrt(force_rmse / force_count))
...
if os.path.exists(TrainingLog) or reset_lr:
    ...
    training_log = pandas.DataFrame(columns=["Energy RMSE", "Force RMSE", "Loss"])
...
for _ in range(AdamW_scheduler.last_epoch + 1, max_epochs):
    energy_rmse, force_rmse = validate()
    EF_coef = energy_rmse + (force_rmse * config["force_coefficient"])
    
    Log.Log(f"ENERGY RMSE: {round(energy_rmse, 3)} FORCE RMSE: {round(force_rmse, 3)} kcal/mol EF_coef: {round(EF_coef, 3)} at epoch {AdamW_scheduler.last_epoch + 1}")
    ...
    for i, properties in enumerate(training): #,desc="epoch {}".format(AdamW_scheduler.last_epoch)
        ...
        training_log.loc[AdamW_scheduler.last_epoch + 1] = [energy_rmse, force_rmse, float(loss.sum().cpu().detach().numpy())] 
    ...
    if (AdamW_scheduler.last_epoch + 1) % config["GraphEveryEpochs"] == 0:
        ...
        axs[1].set_ylabel("Force RMSE (kcal/mol)")
        axs[0].set_ylabel("Energy RMSE (kcal/mol)")
